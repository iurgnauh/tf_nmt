...
def embedding(x, vocab_size, embedding_size):
  """Embed x of type int64 into dense vectors, reducing to max 4 dimensions."""
  with tf.variable_scope('WordEmbedding'):
    C = tf.get_variable('C', [vocab_size, embedding_size], initializer=\
                    tf.random_normal_initializer(0.0, embedding_size**-0.5, dtype=tf.float32))
    emb_x = tf.gather(C, x)
    return emb_x
...
def conv1d(inputs, filters, kernel_size, **kwargs):
  ...
  with tf.variable_scope(name):
    kernel = tf.get_variable('kernel', [H, filters])
    bias = tf.get_variable('bias', [filters], initializer=tf.zeros_initializer())
  shp = tf.shape(inputs)
  inputs = tf.reshape(inputs, [shp[0]*shp[1],shp[2]])
  #inputs = tf.reshape(inputs, [-1,shp[2]])
  outputs = tf.matmul(inputs, kernel) + bias
  return tf.reshape(outputs, [shp[0], shp[1], filters])
...
def conv_hidden_relu(inputs,
  ...
  with tf.variable_scope(name, "conv_hidden_relu", [inputs]):
    ...
    ret = conv1d(h, output_size, second_kernel_size, name="conv2", **kwargs)
    return ret
