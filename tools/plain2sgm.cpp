...
void OutSingleFile(const vector<string>& ifile, const string& ofile,
		const string& dir = "src") {
	//ofstream fout(ofile.c_str());
	if (dir == "src")
		cout << "<srcset setid=\"set\" srclang=\"src\" trglang=\"tgt\">"
				<< endl;
	else if (dir == "ref")
		cout << "<refset setid=\"set\" srclang=\"src\" trglang=\"tgt\">"
				<< endl;
	else if (dir == "tst")
		cout << "<tstset setid=\"set\" srclang=\"src\" trglang=\"tgt\">"
				<< endl;
...
int main(int argc, char** argv) {
	if (argc < 3) {
		...
		cerr
				<< "./plain2sgm ref ref.plain0 ref.plain1 ref.plain2 .... > ref.sgm"
				<< endl;
		cerr << "./plain2sgm tst tst.plain > tst.sgm" << endl;
		exit(1);
	}
	if (string(argv[1]) != "src" && string(argv[1]) != "ref"
			&& string(argv[1]) != "tst") {
		cerr << "[src|ref|tst][plain0][...][plainN]" << endl;
		exit(1);
	}
